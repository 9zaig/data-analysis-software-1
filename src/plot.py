...
class Plot:
    def __init__(self,IV_X,IV_Y,SPC_X,SPC_Y):
        self.IV_X=IV_X
        self.IV.Y=IV_Y
        self.SPC_X=SPC_X
        self.SPC_Y=SPC_Y
    ...
    def IV_ployfit_plot(self):
        plt.plot(self.IV_X,abs(ft.normal_fit()),'r',label='fitting(ployfit)')
        plt.xlabel('Voltage [V]')
        plt.ylabel('Current [A]')
        plt.xticks(np.arange(-2, 1.25, 0.25))
        plt.yticks([10 ** -12, 10 ** -11, 10 ** -10, 10 ** -9, 10 ** -8, 10 ** -7, 10 ** -6, 10 ** -5, 10 ** -4, 10 ** -3,10 ** -2])
    ...
    def IV_lmfit_plot(self):
        plt.plot(self.IV_X,abs(ft.non_linear_fit()),'r',label='fitting(lmfit)')
        plt.xlabel('Voltage [V]')
        plt.ylabel('Current [A]')
        plt.xticks(np.arange(-2, 1.25, 0.25))
        plt.yticks([10 ** -12, 10 ** -11, 10 ** -10, 10 ** -9, 10 ** -8, 10 ** -7, 10 ** -6, 10 ** -5, 10 ** -4, 10 ** -3,10 ** -2])
    ...
    def ref_fit_plot(self):
        x_line = np.arange(min(self.SPC_X[6]), max(self.SPC.X[6]), 0.01)
